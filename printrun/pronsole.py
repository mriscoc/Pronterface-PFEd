...
class pronsole(cmd.Cmd):
    def __init__(self):
        ...
        self.p.callback.recv = self.recvcb
        self.p.callback.start = self.startcb
        self.p.callback.end = self.endcb
        self.p.callback.layerchange = self.layer_change_cb
        self.p.callback.hostcommand = self.process_host_command
        self.recvlisteners = []
        self.in_macro = False
        self.p.callback.online = self.online
        self.p.callback.error = self.logError
        self.fgcode = None
        self.filename = None
